...
4. 对缺失标的、最新交易日落后于全体标的最新交易日的、或最新交易日前一个 NYSE 交易日存在缺口的（如行情源对收盘价的入库滞后或历史回溯删除），依次用 Nasdaq 历史接口、Yahoo 1d 报价管线、Nasdaq 实时报价（最后成交价）补齐；补缺失败会在错误摘要中说明落后标的、所需交易日与各层的失败原因；
5. 为每个配对寻找共同的最近两个交易日；
6. 检查所有活跃配对的日期是否一致；
