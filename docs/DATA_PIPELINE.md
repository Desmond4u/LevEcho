...
4. 对缺失标的，或最新交易日落后于全体标的最新交易日的（如 `yfinance` 历史行情对收盘价的回填滞后），先用 Nasdaq 公共历史行情接口补齐缺失交易日；Nasdaq 不可用时退回 Yahoo 1d 报价管线（`YFinanceRecentProvider`）补最新一根。补缺失败会在错误摘要中说明落后标的、所需交易日与失败原因；
5. 为每个配对寻找共同的最近两个交易日；
6. 检查所有活跃配对的日期是否一致；
...
当前实现会检查共同日期数量、双方最新日期以及所有活跃配对的日期一致性，并通过 NYSE 交易日历（`exchange_calendars`，离线包）验证基准日与最新日严格相邻；节假日与周末跨跃视为相邻，缺失中间行情则直接构建失败。缺失的中间行情不会通过前向填充隐藏。
