...
TEXT = {
    ...
    "formula_help": ("S₀ 为股票基准价，E₀ 为 ETF 基准价，L 为有符号日目标倍数。模型仅用于单日，不适用于跨日累计收益。历史区间取双方最近两个共同行情日期，尚未通过交易所日历确认相邻；公司行为字段缺失不代表已确认无公司行为。", "S₀ is the stock reference price, E₀ the ETF reference price, and L the signed daily leverage. The model applies to one session, not cumulative multi-day returns. Historical dates are the latest two common quote dates; adjacency has not been verified against an exchange calendar. Missing corporate-action fields do not confirm the absence of corporate actions."),
