...
The market-data layer uses `yfinance` as the primary provider, with Nasdaq's history endpoint, Yahoo's 1d quote pipeline and Nasdaq's live quote as fallbacks for missing symbols or ones whose latest session trails the rest. See:
